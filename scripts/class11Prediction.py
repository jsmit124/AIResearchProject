...
if len(sys.argv) >= 2:
    train_data = data[
        ['Age', 'Gender', 'Education', 'Country', 'Ethnicity', 'Nscore', 'Escore', 'Oscore', 'Ascore', 'Cscore',
         'Impulsive', 'SS', 'Class10', 'Class01', 'Class00']]
else:
    train_data = data[
        ['Age', 'Gender', 'Education', 'Country', 'Ethnicity', 'Nscore', 'Escore', 'Oscore', 'Ascore', 'Cscore',
         'Impulsive', 'SS']]
...
def main():
    ...
    if len(sys.argv) >= 2:
        with open('./kFoldResults/crossValidationClass11WithLabelClassesResults.csv', mode='w', newline='') as file:
            writer = csv.writer(file, delimiter=',', )
            for data in range(0, k_scores.qsize()):
                writer.writerow(k_scores.get())
        file.close()
    else:
        with open('./kFoldResults/crossValidationClass11WithoutLabelClassesResults.csv', mode='w', newline='') as file:
            writer = csv.writer(file, delimiter=',', )
            for data in range(0, k_scores.qsize()):
                writer.writerow(k_scores.get())
        file.close()
